...
const EmployeeSchedule: React.FC = () => {
    ...
    const [selectedEmployeeData, setSelectedEmployeeData] = useState<EmployeeWeeklyData | null>(null);
    const [userCalculation, setUserCalculation] = useState<EmployeeRecord | null | undefined>(null);
    ...
    const weeklyData = useMemo(() => {
        ...
        employeeMap.forEach((data) => {
            ...
            data.totalHours = `${totalHours}:${remainingMinutes.toString().padStart(2, '0')}`;
        });
                    ...
                    <table className='min-w-full bg-white border border-neutral-200'>
                        <thead>
                            <tr className='bg-neutral-50'>
                                ...
                                <th className='px-4 py-2 border-b border-r border-neutral-200 text-left'>Employee</th>
                                {weekDays.map((day) => (
                                    <th
                                        key={day.toISOString()}
                                        className={`px-4 py-2 border-b border-r border-neutral-200 ${
                                            day.getDay() === 0 || day.getDay() === 6 ? 'bg-blue-50' : ''
                                        }`}
                                    >
                                        {format(day, 'EEE dd')}
                                    </th>
                                ))}
                                <th className='px-4 py-2 border-b border-neutral-200 bg-blue-100'>Total</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr key={employee.id} className='hover:bg-neutral-50'>
                                    ...
                                    {employee.dailyHours.map((day, index) => (
                                        <td
                                            key={index}
                                            className={`px-4 py-2 border-b border-r border-neutral-200 ${
                                                weekDays[index].getDay() === 0 || weekDays[index].getDay() === 6 ? 'bg-blue-50' : ''
                                            }`}
                                            onClick={(e) => {
                                        ...
                                        >
                                            <div className='flex items-center justify-between'>
                                                <span className={`${day.hasAlert ? 'text-red-500' : ''}`}>{day.hours}</span>
                                                <div
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        const editing = employee.id;
                                                        const editingDate = format(weekDays[index], 'yyyy-MM-dd');
                                                        const user = () => {
                                                            if (!editing || !editingDate) return null;
                                                            const employee = employeeRecords.filter((r) => r.id === editing);
                                                            if (employee.length === 0) return null;
                                                            return employee.find((r) => r.dateId === editingDate);
                                                        };
                ...
                {weeklyData.length === 0 && (
                    <Card title='No Records Found' icon={<FiUsers size={20} />} className='animate-fade-in'>
                        <div className='text-center py-8'>
                            <div className='text-neutral-400 mb-2'>No records found for this week</div>
                            <button
            ...
            <WeeklyBreakdownModal
                ...
                dailyRecords={
                    selectedEmployeeData
                        ? weekDays.map((day) => {
                              const record = filteredRecords.find((r) => r.id === selectedEmployeeData.id && r.date === format(day, 'yyyy-MM-dd'));
                              if (!record) {
                                  return {
                ...
                weeklyTotal={
                    selectedEmployeeData
                        ? parseFloat(selectedEmployeeData.totalHours.split(':')[0]) + parseFloat(selectedEmployeeData.totalHours.split(':')[1]) / 60
                        : 0
                }
            />
        </>
