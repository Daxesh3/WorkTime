...
interface WorkTimeStore {
  ...
  addEmployeeRecord: (
    record: Omit<EmployeeRecord, "id" | "calculatedHours">
  ) => void;
  ...
  persist(
    (set, get) => ({
      ...
      addEmployeeRecord: (record) => {
        const newRecord: EmployeeRecord & { dateId: string } = {
          ...record,
      ...
      updateEmployeeRecord: (id, dateId, updates) => {
        set((state) => ({
          employeeRecords: state.employeeRecords.map((record) =>
            record.id === id && record.dateId === dateId
              ? { ...record, ...updates }
              : record
          ),
        }));
      ...
      deleteEmployeeRecord: (id) => {
        set((state) => ({
          employeeRecords: state.employeeRecords.filter(
            (record) => record.id !== id
          ),
        }));
      },
      simulateCalculation: (record, shift) => {
        ...
        const workEnd = parseTime(
          shift.end,
          shift.start > shift.end ? addMinutes(workStart, 24 * 60) : workStart
        ); // Handle night shifts
        const clockIn = parseTime(record.clockIn);
        const clockOut = parseTime(
          record.clockOut,
          record.clockIn > record.clockOut
            ? addMinutes(clockIn, 24 * 60)
            : clockIn
        ); // Handle night shifts
        ...
        const calculatedHours = format(
          addMinutes(new Date(0).getTime(), totalWorkingMinutes),
          "HH:mm"
        );
        ...
        const overtimePay =
          (overtimeMinutes / 60) * shift.lateStay.overtimeMultiplier;

        // Check lunch break window
        const isLunchBreakInWindow =
          lunchStart >= flexWindowStart && lunchEnd <= flexWindowEnd;
        const isLunchBreakCorrectDuration =
          lunchBreakMinutes === shift.lunchBreak.duration;

        // Calculate early/late arrival and departure
        const earlyArrivalMinutes =
          clockIn < workStart ? differenceInMinutes(workStart, clockIn) : 0;
        const lateArrivalMinutes =
          clockIn > workStart ? differenceInMinutes(clockIn, workStart) : 0;
        const earlyDepartureMinutes =
          clockOut < workEnd ? differenceInMinutes(workEnd, clockOut) : 0;
        const lateDepartureMinutes =
          clockOut > workEnd ? differenceInMinutes(clockOut, workEnd) : 0;
        ...
        return {
          ...
          totalWorkingMinutes,
          regularHours: formatDurationManually(
            differenceInMinutes(workEnd, workStart) - shift.lunchBreak.duration
          ),
          overtimeHours,
